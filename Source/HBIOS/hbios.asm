...
#ENDIF
;
; NB: ROMWBW USES 32K PAGES, ZEAL 8-BIT USES 16K PAGES
#IF (MEMMGR == MM_Z8B)
        RLCA                            ; WITH 16K WE HAVE TWICE MORE PAGES, SO MULTIPLY BY 2
        RES	0,A			; CLEAR BIT POSSIBLY AFFECTED BY RLCA
	JR	NC,HBX_ROM		; IF BIT 7 WASN'T SET, THEN IT'S ROM
        ADD     A, $80000>>14           ; ADD RAM FIRST PAGE INDEX
HBX_ROM:
	OUT	(Z8B_MMU_PAGE0),A       ; BANK_0: 0K - 16K
	INC	A			;
	OUT	(Z8B_MMU_PAGE1),A	; BANK_1: 16K - 32K
	RET				; DONE
#ENDIF
;
#IF (MEMMGR == MM_N8)
...
#ENDIF
;
; ZEAL 8-BIT MMU INITIALIZATION
;
#IF (MEMMGR == MM_Z8B)
  #IFDEF ROMBOOT
	; IF THIS IS A ROM BOOT, SETUP THE FIRST 2 16K MMU REGISTERS
	; TO MAP THE FIRST 32K OF PHYSICAL ROM TO THE LOW 32K OF
	; CPU ADDRESS SPACE (BANKING AREA).  THE FIRST 16K MAPPING IS
	; REDUNDANT BECAUSE WE ARE ALREADY RUNNING IN THIS AREA.
        XOR     A
	OUT	(Z8B_MMU_PAGE0),A       ; BANK_0: 0K - 16K
        INC     A
	OUT	(Z8B_MMU_PAGE1),A       ; BANK_1: 16K - 32K
  #ENDIF
	LD	A, ($100000>>14)-2      ; LAST TWO PAGES OF RAM
	OUT	(Z8B_MMU_PAGE2),A       ; BANK_2: 32K - 48K
        INC     A
	OUT	(Z8B_MMU_PAGE3),A       ; BANK_3: 32K - 48K
#ENDIF
;
;
;
